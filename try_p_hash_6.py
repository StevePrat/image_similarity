# -*- coding: utf-8 -*
# p hash calculation using multiprocessing pool.imap
# writing into file is included under parallel processing
# create separate output for newly calculated hashes
# input file includes shop_id
...
NEW_HASH_FOLDER = HOME_PATH + 'new_hash_{}/'.format(datetime.date.today())
TO_CHECK_FOLDER = check_folder_name(options.folder)
REGION = options.reg
...
class OutputFileLock:
    def __init__(self) -> None:
        self.synchronizing_array: SynchronizedArray = Array('B', 16**4)
    
    def get_file_status(self, img_id_head: str):
        array_index = int(img_id_head, 16)
        return self.synchronizing_array[array_index]

    def set_file_status(self, img_id_head: str, new_value: int) -> None:
        array_index = int(img_id_head, 16)
        self.synchronizing_array[array_index] = new_value

    def acquire_file(self, img_id_head: str) -> None:
        while self.get_file_status(img_id_head) == 1:
            pass
        self.set_file_status(img_id_head, 1)
    
    def release_file(self, img_id_head: str) -> None:
        self.set_file_status(img_id_head, 0)

STORED_HASH_LOCK = OutputFileLock()
NEW_HASH_LOCK = OutputFileLock()
...
def udpate_output_file(img_hash_map: Dict[str, imagehash.ImageHash], folder: str, output_file_lock: OutputFileLock) -> None:
    ...
    for img_id_head, img_id_subset in img_id_grp.items():
        file_path = folder + img_id_head + '.csv'
        
        output_file_lock.acquire_file(img_id_head)
        ...
        output_file_lock.release_file(img_id_head)
...
def img_hash_processing_function(img_id_list: List[str]) -> Dict[str, imagehash.ImageHash]:
    ...
    for i, img in enumerate(new_images):
        if isinstance(img, Image.Image):
            try:
                img_hash = imagehash.phash(img, 16)
            except Exception as e:
    ...
    udpate_output_file(img_hash_map, STORED_HASH_FOLDER, STORED_HASH_LOCK)
    udpate_output_file(img_hash_map, NEW_HASH_FOLDER, NEW_HASH_LOCK)
    return img_hash_map

def batch_img_id_generator(file_name_list: Iterable[str], split_size: int) -> Generator[List[str], None, None]:
    for file_name in file_name_list:
        print('Opening', file_name)
        df = pd.read_csv(TO_CHECK_FOLDER + file_name, names=['grass_region','shop_id','item_id','img_id'])
        df: pd.DataFrame= df[df['img_id'].map(lambda x: set(str(x).lower()).issubset(set('0123456789abcdef')))]
        img_id_array: np.ndarray = df['img_id'].unique().astype(str)
        img_id_array.sort()
        img_id_list: List[str] = img_id_array.tolist()
...
def main() -> None:
    ...
    if not os.path.exists(NEW_HASH_FOLDER):
        os.mkdir(NEW_HASH_FOLDER)
